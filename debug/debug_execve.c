...
static void inline fixup_arg_str(char *arg_ptr, int cur_argc,
                                 const char *what) {
  pr_loc_wrn("Failed to copy %d arg - %s failed", cur_argc, what);
  memcpy(arg_ptr, "..?\0", 4);
}

void RPDBG_print_execve_call(const char *filename,
  ...
  for (int i = 0; i < argc; i++) {
    ...
    --len; // we want to copy without nullbyte as we handle it ourselves while
           // attaching to arg_ptr
    ...
    *arg_ptr = (i + 1 == argc)
                   ? '\0'
                   : ' '; // separate by spaces UNLESS it's the last argument
    ++arg_ptr;
  }

out_free:
  pr_loc_dbg("execve@cpu%d: %s[%d]=>%s[%d] {%s}", caller->on_cpu, caller->comm,
             caller->pid, filename, argc, arg_str);
  kfree(arg_str);
}