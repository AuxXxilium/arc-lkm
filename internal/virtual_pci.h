...
/*
 * The following macros are useful for converting PCI_CLASS_* constants to
 * individual values in structs 31 .................. 0 [class][sub][prog][rev]
...
// This struct MUST be packed to allow for easy reading, see
// https://kernelnewbies.org/DataAlignment
struct pci_dev_descriptor {
  u16 vid; // Vendor ID
  u16 dev; // Device ID

  u16 command; // see PCI_COMMAND_*. Simply do "dev.command |= PCI_COMMAND_xxx"
               // to set a flag.
  u16 status;  // see PCI_STATUS_*. Simply do "dev.status |= PCI_STATUS_xxx" to
               // set a flag.

  u8 rev_id;
  u8 prog_if;  // ]
  u8 subclass; // ]-> prof_if, subclass, and class are normally represented as
               // 24-bit class code
  u8 class;    // ]
...
extern const struct pci_dev_descriptor
    pci_dev_conf_default_normal_dev; // See details in the .c file
...
struct pci_dev_capability {
  ...
  u8 cap_next; // offset where next capability exists, set to 0x00 to denote
               // null-capability
  u8 cap_data[];
} __packed;
 ...
 *
 * If you don't want to create the descriptor from scratch you can use "const
 * struct pci_dev_conf_default_normal_dev" while setting some missing params
 * (see .c file header for details). Note: you CAN reuse the same descriptor
 * under multiple BDFs (bus_no/dev_no/fn_no)
 *
 ...
 *  - this function has a slight limitation due to how Linux scans devices. You
 * HAVE TO add fn_no=0 entry as the LAST one when calling it multiple times.
 * Kernel scans devices only once for changes and if it finds fn=0 and it's the
 *    only one (i.e. you added fn=0 first) adding more functions will not
 * populate them (as kernel will never re-scan the device).
 *  - As per PCI spec Linux doesn't allow devices to have fn>0 if they don't
 * have corresponding fn=0 entry
 *
 * @param bus_no (0x00 - 0xFF)
 * @param dev_no (0x00 - 0x20)
 ...
 *
 * Known bug: while you can remove things and they will be gone from the system
 * you CANNOT re-add the same under the same BFD coordinates. This will cause
 * the kernel to complain about duplicated internal sysfs entries. It's most
