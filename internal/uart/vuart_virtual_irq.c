...
/**
 * Function running on a separate kernel thread responsible for simulating the
 * IRQ call (normally done via hardware interrupt triggering CPU to invoke Linux
 * IRQ subsystem)
 *
 * There's no sane way to trigger IRQs in the low range used by 8250 UARTs. A
 * pure asm call of "int $4" will result in a crash (yes, we did try first ;)).
 * So instead of hacking around the kernel we simply used the 8250 public
 * interface to trigger interrupt routines and implemented a small IRQ handling
 * subsystem on our own.
 * @param data
 * @return
 */
static int virq_thread(void *data) {
  ...
  uart_prdbg("%s started for ttyS%d pid=%d", __FUNCTION__, vdev->line,
             current->pid);
  while (likely(!kthread_should_stop())) {
    wait_event_interruptible(*vdev->virq_queue,
                             !(vdev->iir & UART_IIR_NO_INT) ||
                                 unlikely(kthread_should_stop()));
    if (unlikely(signal_pending(current))) {
      uart_prdbg("%s started for ttyS%d pid=%d received signal", __FUNCTION__,
                 vdev->line, current->pid);
      out = -EPIPE;
      break;
    }
    ...
    if (unlikely(!vdev->up)) {
      pr_loc_bug("Cannot call serial8250 interrupt handler - port not captured "
                 "(yet?)");
      continue;
    }
  ...
  uart_prdbg("%s stopped for ttyS%d pid=%d exit=%d", __FUNCTION__, vdev->line,
             current->pid, out);

  // that can lead to a small memory leak for virq_queue if thread is killed
  // outisde disable_interrupts() but this
...
int vuart_enable_interrupts(struct serial8250_16550A_vdev *vdev) {
  ...
  if (unlikely(vuart_virq_active(vdev))) {
    pr_loc_bug("Interrupts are already enabled & scheduled for ttyS%d",
               vdev->line);
    out = -EBUSY;
    goto error_unlock_free;
  }
  ...
  if (!(vdev->virq_queue = kmalloc(sizeof(wait_queue_head_t), GFP_KERNEL)) ||
    ...
    out = -ENOMEM;
    pr_loc_crt("kernel memory alloc failure - tried to reserve memory for vIRQ "
               "structures");
    goto error_unlock_free;
  }

  init_waitqueue_head(vdev->virq_queue);
  unlock_vuart(vdev); // we can safely unlock after reserving memory but before
                      // starting thread (so we're not atomic)

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-extra-args"
  // VUART_THREAD_FMT can resolve to anonymized version without line or even
  // IRQ#
  vdev->virq_thread =
      kthread_run(virq_thread, vdev, VUART_THREAD_FMT, vdev->irq, vdev->line);
