/*
 * Submodule used to hook the execve() syscall, used by the userland to execute
 * binaries.
 *
 * This submodule can currently block calls to specific binaries and fake a
 * successful return of the execution. In the future, if needed, an option to
 * fake certain response and/or execute a different binary instead can be easily
 * added here.
 *
 * execve() is a rather special syscall. This submodule utilized
 * override_symbool.c:override_syscall() to do the actual ground work of
 * replacing the call. However some syscalls (execve, fork, etc.) use ASM stubs
 * with a non-GCC call convention. Up until Linux v3.18 it wasn't a problem as
 * long as the stub was called back. However, since v3.18 the stub was changed
 * in such a way that calling it using a normal convention from (i.e. from the
 * shim here) will cause IRET imbalance and a crash. This is worked around by
 * skipping the whole stub and calling do_execve() with a filename struct
 * directly. This requires re-exported versions of these functions, so it may be
 * marginally slower. Because of that this trick is only utilized on Linux
 * >v3.18 and older ones call the stub as normal.
 *
 * References:
 *  -
...
int add_blocked_execve_filename(const char *filename) {
  ...
  while (likely(intercepted_filenames[idx])) { // Find free spot
    if (unlikely(strcmp(filename, intercepted_filenames[idx]) ==
                 0)) { // Does it exist already?
      pr_loc_bug("File %s was already added at %d", filename, idx);
      return -EEXIST;
    }

    if (unlikely(++idx >= MAX_INTERCEPTED_FILES)) { // Are we out of indexes?
      pr_loc_bug("Tried to add %d intercepted filename (max=%d)", idx,
                 MAX_INTERCEPTED_FILES);
      return -ENOMEM;
    }
  }
...
SYSCALL_SHIM_DEFINE3(execve, const char __user *, filename,
                     const char __user *const __user *, argv,
                     const char __user *const __user *, envp) {
  struct filename *path = _getname(filename);

  // this is essentially what do_execve() (or SYSCALL_DEFINE3 on older kernels)
  // will do if the getname ptr is invalid
  if (IS_ERR(path))
    return PTR_ERR(path);
  ...
  for (int i = 0; i < MAX_INTERCEPTED_FILES; i++) {
    ...
    if (unlikely(strcmp(pathname, intercepted_filenames[i]) == 0)) {
      ...
      // We cannot just return 0 here - execve() *does NOT* return on success,
      // but replaces the current process ctx
      do_exit(0);
    }
  }

// Depending on the version of the kernel do_execve() accepts bare filename
// (old) or the full struct filename (newer) Additionally in older kernels we
// need to take care of the path lifetime and put it back (it's automatic in
// newer) See:
...
int register_execve_interceptor() {
  ...
  if (sys_execve_ovs) {
    pr_loc_bug("Called %s() while execve() interceptor is already registered",
               __FUNCTION__);
    return -EEXIST;
  }
  ...
  override_symbol_or_exit_int(sys_execve_ovs, "__x64_sys_execve",
                              SyS_execve_shim);
#endif
...
int unregister_execve_interceptor() {
  ...
  if (!sys_execve_ovs) {
    pr_loc_bug(
        "Called %s() while execve() interceptor is not registered (yet?)",
        __FUNCTION__);
    return -ENXIO;
  }
