 ...
 *
 * @return pointer to the spot on the list containing driver_watcher_instance or
 * NULL if not found
 */
...
/**
 * Checks if there any watchers registered (to determine if it makes sense to
 * still shim the driver_register())
 */
...
static int call_original_driver_register(struct device_driver *drv) {
  ...
  org_call_out =
      call_overridden_symbol(driver_register_out, ov_driver_register, drv);

  if (unlikely(org_call_out != 0)) {
    pr_loc_err("Failed to call original %s (error=%d)", WATCH_FUNCTION,
               org_call_out);
    return org_call_out;
  }
...
static int driver_register_shim(struct device_driver *drv) {
  ...
  if (unlikely(!watcher_lptr)) {
    pr_loc_dbg("%s() interception active - no handler observing \"%s\" found, "
               "calling original %s()",
               WATCH_FUNCTION, drv->name, WATCH_FUNCTION);
    return call_original_driver_register(drv);
  }

  pr_loc_dbg("%s() interception active - calling handler %pF<%p> for \"%s\"",
  ...
  if ((*watcher_lptr)->notify_coming) {
    ...
    // CONTINUE and DONE cannot use fall-through as we cannot unregister watcher
    // before calling it (as if this is the
    //  last watcher the whole override will be stopped
    case DWATCH_NOTIFY_CONTINUE:
      pr_loc_dbg("Calling original %s() & leaving watcher active",
                 WATCH_FUNCTION);
    ...
    case DWATCH_NOTIFY_DONE:
      ...
      driver_load_result = call_original_driver_register(drv);
      unwatch_driver_register(
          *watcher_lptr);        // regardless of the call result we unregister
      return driver_load_result; // we return here as the watcher doesn't want
                                 // to be bothered anymore
    case DWATCH_NOTIFY_ABORT_OK:
      pr_loc_dbg("Faking OK return of %s() per callback request",
                 WATCH_FUNCTION);
      driver_load_result = 0;
      driver_register_fulfilled = true;
      break;
    case DWATCH_NOTIFY_ABORT_BUSY:
      pr_loc_dbg("Faking BUSY return of %s() per callback request",
                 WATCH_FUNCTION);
      driver_load_result = -EBUSY;
      driver_register_fulfilled = true;
      break;
    default: // This should never happen if the callback is correct
      pr_loc_bug("%s callback %pF<%p> returned invalid status value during "
                 "DWATCH_STATE_COMING",
                 WATCH_FUNCTION, (*watcher_lptr)->cb, (*watcher_lptr)->cb);
    }
  }
  ...
  if (driver_load_result != 0) {
    pr_loc_err("%s driver failed to load - not triggering STATE_LIVE callbacks",
               drv->name);
...
static int start_watching(void) {
  ...
  if (unlikely(IS_ERR(ov_driver_register))) {
    pr_loc_err("Failed to intercept %s() - error=%ld", WATCH_FUNCTION,
               PTR_ERR(ov_driver_register));
...
static int stop_watching(void) {
  ...
  if (unlikely(out != 0)) {
    pr_loc_err("Failed to restore %s() - error=%ld", WATCH_FUNCTION,
               PTR_ERR(ov_driver_register));
    return out;
  }
...
driver_watcher_instance *
watch_driver_register(const char *name, watch_dr_callback *cb, int event_mask) {
  driver_watcher_instance **watcher_lptr = match_watcher(name);
  if (unlikely(watcher_lptr)) {
    pr_loc_err("Watcher for %s already exists (callback=%pF<%p>)", name,
               (*watcher_lptr)->cb, (*watcher_lptr)->cb);
  ...
  kmalloc_or_exit_ptr(*watcher_lptr,
                      sizeof(driver_watcher_instance) + strsize(name));
  strcpy((*watcher_lptr)->name, name);
  (*watcher_lptr)->cb = cb;
  (*watcher_lptr)->notify_coming =
      ((event_mask & DWATCH_STATE_COMING) == DWATCH_STATE_COMING);
  (*watcher_lptr)->notify_live =
      ((event_mask & DWATCH_STATE_LIVE) == DWATCH_STATE_LIVE);
  pr_loc_dbg("Registered %s() watcher for \"%s\" driver (coming=%d, live=%d)",
             WATCH_FUNCTION, name, (*watcher_lptr)->notify_coming ? 1 : 0,
             (*watcher_lptr)->notify_live ? 1 : 0);

  if (!ov_driver_register) {
    pr_loc_dbg(
        "Registered the first driver_register watcher - starting watching");
...
int unwatch_driver_register(driver_watcher_instance *instance) {
  ...
  if (unlikely(!matched_lptr)) {
    // This means it could be a double-unwatch situation and this will prevent a
    // double-kfree (but the lack of crash
    //  is not guaranteed as match_watcher() already touched the memory)
    pr_loc_bug("Watcher %p for %s couldn't be found in the watchers list",
               instance, instance->name);
    return -ENOENT;
  }

  if (unlikely(*matched_lptr != instance)) {
    pr_loc_bug("Watcher %p for %s was found but the instance on the list %p "
               "(@%p) isn't the same (?!)",
               instance, instance->name, *matched_lptr, matched_lptr);
    return -EINVAL;
  }

  pr_loc_dbg("Removed %pF<%p> subscriber for \"%s\" driver",
             (*matched_lptr)->cb, (*matched_lptr)->cb, (*matched_lptr)->name);
  kfree(*matched_lptr);
  *matched_lptr = NULL;

  if (!has_any_watchers()) {
    pr_loc_dbg("Removed last %s() subscriber - unshimming %s()", WATCH_FUNCTION,
               WATCH_FUNCTION);
    int out;
