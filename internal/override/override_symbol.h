...
#define call_overridden_symbol_void(sym, ...)                                  \
  ({                                                                           \
    ...
    _Pragma("GCC diagnostic push") _Pragma(                                    \
        "GCC diagnostic ignored \"-Wstrict-prototypes\"") void (*__ptr)() =    \
        __get_org_ptr(sym);                                                    \
    _Pragma("GCC diagnostic pop") __ret = __disable_symbol_override(sym);      \
    if (likely(__ret == 0)) {                                                  \
      __ptr(__VA_ARGS__);                                                      \
 ...
 *
 * @param out_var name of the variable where original function return value
 * should be placed
 * @param sym pointer to a override_symbol_inst
 * @param ... any arguments to the original function
 *
  ...
  ({                                                                           \
    ...
    _Pragma("GCC diagnostic push")                                             \
        _Pragma("GCC diagnostic ignored "                                      \
                "\"-Wstrict-prototypes\"") typeof(out_var) (*__ptr)() =        \
            __get_org_ptr(sym);                                                \
    _Pragma("GCC diagnostic pop") __ret = __disable_symbol_override(sym);      \
    if (likely(__ret == 0)) {                                                  \
...
/**
 * override_symbol() with automatic error handling. See the original function
 * for details.
 *
 ...
 *
 * @return Instance of override_symbol_inst struct pointer on success,
 * ERR_PTR(-E) on error
 *
 * @example
 ...
 *     ...
 *     restore_symbol(backup_addr, backup_code); //restore backed-up copy of
 * printk()
 *
 * @todo: This should be rewritten using INSN without inline ASM wizardy, but
...
/**
 * Frees the symbol previously reserved by get_ov_symbol_instance() (e.g. via
 * override_symbol)
 *
 * !! READ THIS SERIOUS WARNING BELOW CAREFULLY !!
 * STOP! DO NOT USE THIS if you don't understand what it does and why it exists.
 * This function should be called from outside of this submodule ONLY if the
 * overridden code disappeared from memory. This practically can happen only
 * when you override a symbol inside of a loadable module and the module is
 * unloaded. In ANY other case you must call restore_symbol() to actually
 * restore the original code. This function simply "forgets" about the override
 * and frees memory (as if external module has been unloaded we are NOT allowed
 * to touch that memory anymore as it may be freed). It is explicitly NOT
 * necessary to call this function after restore_symbol() as it does so
 * internally.
 */
...
/****************** Private helpers (should not be used directly by any code
