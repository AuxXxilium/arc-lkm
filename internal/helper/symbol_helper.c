...
#else
/*
 * In kernel version 5.7, kallsyms_lookup_name() was unexported, so we can't use
 * it anymore. The alternative method below is slower (but not really
 * noticably), and works by brute-forcing possible addresses for the function
 * name by starting at the kernel base address and using sprint_symbol() (which
 * is still exported) to check if the symbol name at each address matches the
 * one we want.
 *
...
static unsigned long kaddr_lookup_name(const char *fname_raw) {
   ...
   * are within the first 0x100000 bytes of the base address. However, the
   * kernel functions are all aligned so that the final nibble is 0x0, so we
   * only have to check every 16th address.
   */
...
int get_kln_p(void) {
  kln_func = (long unsigned int (*)(const char *))kaddr_lookup_name(
