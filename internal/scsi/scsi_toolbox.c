 ...
 * @return 0 on command success, >0 if command failed; if the command failed it
 * MAY be repeated
 */
static int scsi_read_cap16(struct scsi_device *sdp, unsigned char *buffer,
  ...
  return scsi_execute_req(sdp, cmd, DMA_FROM_DEVICE, buffer, SCSI_RC16_LEN,
                          sshdr, SCSI_CMD_TIMEOUT, SCSI_CMD_MAX_RETRIES, NULL);
}
 ...
 * MAY be repeated
 */
static int scsi_read_cap10(struct scsi_device *sdp, unsigned char *buffer,
                           struct scsi_sense_hdr *sshdr) {
  unsigned char cmd[16];
  cmd[0] = READ_CAPACITY;
  memset(&cmd[1], 0, 9);
  ...
  return scsi_execute_req(sdp, cmd, DMA_FROM_DEVICE, buffer, 8, sshdr,
                          SCSI_CMD_TIMEOUT, SCSI_CMD_MAX_RETRIES, NULL);
}

long long opportunistic_read_capacity(struct scsi_device *sdp) {
  // some drives work only with the 16 version but older ones can only accept
  // the older variant to prevent false-positive "command failed" we need to try
  // both
  bool use_cap16 = true;
  ...
  do {
    // It can return 0 or a positive integer; 0 means immediate success where 1
    // means an error. Depending on the error the command may be repeated.
    out = (use_cap16) ? scsi_read_cap16(sdp, buffer, &sshdr)
                      : scsi_read_cap10(sdp, buffer, &sshdr);
    if (out == 0)
      break; // command just succeeded

    if (unlikely(
            out >
            0)) { // it's technically an error but we may be able to recover
      if (use_cap16) { // if we previously used CAP(16) and it failed we can try
                       // older CAP(10) [even on hard-fail]
        use_cap16 = false;
        continue;
      }

      // Some failures are hard-failure (e.g. drive doesn't support the cmd),
      // some are soft-failures In soft failures some are known to take more
      // time (e.g. spinning rust is spinning up) and some should be
      // fast-repeat. We really only distinguish hard from soft and just wait
      // some time for others In a normal scenario this path will be cold as the
      // drive will respond to CAP(16) or CAP(10) right away.
      ...
      // Drive deliberately rejected the request and indicated that this
      // situtation will not change
      ...
      if (sshdr.sense_key == UNIT_ATTENTION && sshdr.asc == 0x29 &&
          sshdr.ascq == 0x00) {
        pr_loc_dbg("Drive busy during capacity pre-read (%d attempts left), "
                   "trying again",
                   read_retry - 1);
  ...
  if (out != 0) {
    pr_loc_err("Failed to pre-read capacity of the drive after %d attempts due "
               "to SCSI errors",
  ...
  long long size_mb = ((lba + 1) * sector_size) / 1024 /
                      1024; // sectors * sector size = size in bytes
...
bool is_sata_disk(struct device *dev) {
  // from the kernel's pov SCSI devices include SCSI hosts, "leaf" devices, and
  // others - this filters real SCSI devices
  if (!is_scsi_leaf(dev))
    return false;
  ...
  // more than that use syno's private property (hey! not all of their kernel
  // mods are bad ;)) to determine port which a given device uses (vanilla
  // kernel doesn't care about silly ports - SCSI is SCSI)
  if (!is_scsi_disk(sdp) ||
...
int scsi_force_replug(scsi_device *sdp) {
  ...
  if (unlikely(host->transportt->user_scan)) {
    ...
    return host->transportt->user_scan(host, SCAN_WILD_CARD, SCAN_WILD_CARD,
                                       SCAN_WILD_CARD);
  } else {
    pr_loc_dbg("Triggering generic rescan of host%d", host->host_no);
    // this is unfortunately defined in scsi_scan.c, it can be emulated because
    // it's just bunch of loops, but why? This will also most likely never be
    // used anyway
    return _scsi_scan_host_selected(host, SCAN_WILD_CARD, SCAN_WILD_CARD,
                                    SCAN_WILD_CARD, 1);
  }
}

// We assume that if the sd was loaded once it will never unload (as on most
...
static int inline for_each_scsi_x(on_scsi_device_cb *cb,
                                  int (*filter)(struct device *dev,
                                                on_scsi_device_cb cb)) {
  if (!is_scsi_driver_loaded())
    return -ENXIO;

  int code = bus_for_each_dev(&scsi_bus_type, NULL, cb,
                              (int (*)(struct device *, void *))filter);
