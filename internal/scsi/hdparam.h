/**
 * This file serves a similar role to include/uapi/linux/hdreg.h, combining
 * constants & macros for dealing with ATA
 */
...
/********************************** Parameters related to HDIO_DRIVE_CMD ioctl
 * call ***********************************/
// the following constants contain size/offsets/indexes of "struct
// hd_drive_cmd_hdr" fields from user API
//  also see HDIO_DRIVE_CMD in
//  https://www.kernel.org/doc/Documentation/ioctl/hdio.txt
#define HDIO_DRIVE_CMD_HDR_OFFSET                                              \
  4 // same as HDIO_DRIVE_CMD_HDR_SIZE but in sectors
...
// convert sector size of data section of an ATA (sub)command to the full buffer
// size with header
#define ata_ioctl_buf_size(data_sectors)                                       \
  (u16)(HDIO_DRIVE_CMD_HDR_SIZE +                                              \
        ((data_sectors) * (ATA_SECT_SIZE * sizeof(u8))))
...
/********************************** Parameters related to HDIO_DRIVE_TASK ioctl
 * call **********************************/
...
#define WIN_CMD_SMART                                                          \
  0xb0 // defined in full linux headers as WIN_SMART in hdreg.h
#define WIN_FT_SMART_IMMEDIATE_OFFLINE 0xd4
#define WIN_FT_SMART_READ_LOG_SECTOR 0xd5
#define WIN_FT_SMART_STATUS 0xda
#define WIN_FT_SMART_AUTOSAVE                                                  \
  0xd2 // this is not a typo (AUTOSAVE and AUTO_OFFLINE are spelled differently
       // in ATA spec)
#define WIN_FT_SMART_AUTO_OFFLINE 0xdb

/*************************************** Params related to ATA IDENTIFY command
 * ***************************************/
// Word numbers for the ATA IDENTIFY command response fields & bits in them
// (described in "struct hd_driveid")
#define ATA_ID_COMMAND_SET_1_SMART                                             \
  0x01 // first bit of command set #1 contains SMART supported flag
#define ATA_ID_COMMAND_SET_2_VALID                                             \
  0x4000 // 14th bit with should always be 1 when disk supports cmd set 2
#define ATA_ID_CFS_ENABLE_1_SMART                                              \
  0x01 // first bit of command set #1 contains SMART enable flag
#define ATA_ID_CSF_DEFAULT_VALID                                               \
  0x4000 // 14th bit with should always be 1 when disk supports that

/************************************************* ATA IDENTIFY macros
 * ************************************************/
// These can be used with ATA IDENTIFY data returned by HDIO_GET_IDENTITY or
// HDIO_DRIVE_CMD=>ATA_CMD_IDENTIFY_DEV with
//  when buffer is corrected by HDIO_DRIVE_CMD_HDR_OFFSET (to move over the
//  header data)
#define ata_is_smart_supported(id_data)                                        \
  (((id_data)[ATA_ID_COMMAND_SET_2] & ATA_ID_COMMAND_SET_2_VALID) &&           \
   ((id_data)[ATA_ID_COMMAND_SET_1] & ATA_ID_COMMAND_SET_1_SMART))

#define ata_is_smart_enabled(id_data)                                          \
  (((id_data)[ATA_ID_CSF_DEFAULT] & ATA_ID_CSF_DEFAULT_VALID) &&               \
   ((id_data)[ATA_ID_CFS_ENABLE_1] & ATA_ID_CFS_ENABLE_1_SMART))

// set_/unset_ are deliberately asymmetrical here - we don't want to invalidate
// whole word when disabling SMART
 ...
 * **********************************************/
#define ATA_SMART_RECORD_LEN                                                   \
  12 // length of the SMART snapshot data row in bytes, defined

// Modified for kernel use - it's the "hd_driveid" struct from Linux
// include/uapi/linux/hdreg.h which represents a
//  response to HDIO_GET_IDENTITY. See "Table 26 − IDENTIFY DEVICE information"
//  in ATA/ATAPI-6 spec for details.
