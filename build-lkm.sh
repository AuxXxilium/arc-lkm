    ...
    for TARGET in dev prod; do
      ...
      if [ $docker_exit_code -eq 0 ]; then
        ...
        if [ -f "${FILE_KO}" ]; then
          ...
          # Compress and copy
          if gzip -9 -c "${FILE_KO}" > "${RELEASES_DIR}/rp-${PLATFORM}-${version}-${KERNEL_VER}-${TARGET}.ko.gz" 2>/dev/null; then
            log_info "✓ Created: ${PLATFORM}-${version}-${KERNEL_VER}-${TARGET}"
            ((SUCCESSFUL++))
            FOUND=1
          else
          ...
          log_warn "⚠ ${PLATFORM}/redpill.ko not found"
        fi
