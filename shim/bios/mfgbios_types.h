 ...
 *
 * These are not original types used by the mfg BIOS, but rather recreations
 * from available documentation unless available in GPLed source (in which case
 * they're #include'd)
 */
...
// These aren't defined in synobios.h (while sensors from other categories
// are... go figure)
...
typedef int (*mfgc_get_fan_state_cb)(
    int, enum MfgCompatFanStatus *); // int f(int, MfgCompatFanStatus *)
typedef int (*mfgc_set_fan_state_cb)(
    enum MfgCompatFanStatus,
    enum MfgCompatFanSpeed); // int f(MfgCompatFanStatus, MfgCompatFanSpeed)
typedef int (*mfgc_hwmon_sensor_cb)(
    SYNO_HWMON_SENSOR_TYPE *); // int f(SYNO_HWMON_SENSOR_TYPE *)
...
// List of known indexes in the mfgBIOS vtable. The table can be recovered by
// shim/bios_shim.c. Some of its entries are
...
#define VTK_GET_FAN_STATE                                                      \
  6 // Sig: int f(int, MfgCompatFanStatus *) | present in: DS918+; not: DS3615xs
#define VTK_SET_FAN_STATE 7 // Sig: int f(MfgCompatFanStatus, MfgCompatFanSpeed)
#define VTK_GET_SYS_TEMP                                                       \
  8 // Sig: int f(SYNO_THERMAL_TEMP *) | present in: DS3615xs; not: DS918+
...
#define VTK_RTC_SET_APWR                                                       \
  18 // Sig: int f(MfgCompatAutoPwrOn *) | set auto power on
#define VTK_RTC_GET_APWR                                                       \
  19 // Sig: int f(MfgCompatAutoPwrOn *) | get auto power on
#define VTK_RTC_INT_APWR                                                       \
  20 // Sig: int f(void) | initialize auto power on. present in: DS918+; not:
     // DS3615xs
#define VTK_RTC_UINT_APWR                                                      \
  21 // Sig: int f(void) | uninitialize auto power on. present in: DS918+; not:
     // DS3615xs
...
#define VTK_GET_BKPLANE_STATUS                                                 \
  26 // Sig: int f(MfgCompatBackplaneStatus *) | backplane status
#define VTK_INT_MOD_TPE 27
#define VTK_UNINIT 28 // Sig: int f(void)
#define VTK_SET_CPU_FAN_STATUS                                                 \
  29 // Sig: int f(MfgCompatFanStatus, MfgCompatFanSpeed)
#define VTK_SET_PHY_LED                                                        \
  30 // Sig: int f(MfgCompatGenericLedState) | present in: DS620; not: DS3615xs,
     // DS918+
...
#define VTK_SET_HA_LED                                                         \
  41 // present in: RC18015xs+ (and other HA units? don't have other); not:
     // DS3615xs,DS918+
#define VTK_GET_CPY_BTN                                                        \
  42 // Sig: MfgCompatCopyBtnState f(void) | present in: DS718+

#if RP_MODULE_TARGET_VER == 6
#define VTK_SET_SAFE_REMOVE_LED                                                \
  43 // Sig: int f(bool) | present in: DS3615xs; not: DS918+
...
#elif RP_MODULE_TARGET_VER ==                                                  \
    7 // moved VTK_SET_SAFE_REMOVE_LED and VTK_GET_SYS_CURRENT below HWMON_
      // stuff (sic!)
...
#define VTK_SET_SAFE_REMOVE_LED                                                \
  49 // Sig: int f(bool) | present in: DS3615xs; not: DS918+
