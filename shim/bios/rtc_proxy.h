 ...
 *
 * This is not REALLY implemented and only shimmed. Many motherboards don't
 * handle it well or only support it from certain ACPI PSTATEs. It is even more
 * unsupported by hypervisors. If you REALLY need it create a bug report or a
 * PR.
 */
int rtc_proxy_init_auto_power_on(void);

/**
 * Gets time for auto-power on (shims VTK_RTC_GET_APWR). **See note for
 * rtc_proxy_init_auto_power_on()**
 */
int rtc_proxy_get_auto_power_on(struct MfgCompatAutoPwrOn *mfgPwrOn);

/**
 * Sets time for auto-power on (shims VTK_RTC_SET_APWR). **See note for
 * rtc_proxy_init_auto_power_on()**
 */
int rtc_proxy_set_auto_power_on(struct MfgCompatAutoPwrOn *mfgPwrOn);

/**
 * Disables auto-power on functionality (shims VTK_RTC_UINT_APWR). **See note
 * for rtc_proxy_init_auto_power_on()**
 */
