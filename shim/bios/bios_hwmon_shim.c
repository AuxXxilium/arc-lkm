 ...
 *
 * This submodule emulates both legitimate HWMON calls as well as "legacy"
 * hardware monitoring calls get_fan_status()
 */
...
/************************************* Standards for generating fake sensor
 * readings **********************************/
...
// These percentages are precalculated as we cannot use FPU [safely and easily]
// in kernel space
...
/************************************* Maps between hwmon sensor types & their
 * names **********************************/
...
static const int hwmon_sys_vsens_type_base[][2] = {
    ...
    [HWMON_SYS_VSENS_VCC_ID] =
        {FAKE_V12_MIN, FAKE_V12_MAX}, // todo: this is probably per-model
    [HWMON_SYS_VSENS_VPP_ID] =
        {100, 500}, // todo: if this is really peak-to-peak it should be small
...
// todo: it's defined as __used as we know the structure but don't implement it
// yet
...
#define guard_hwmon_cfg()                                                      \
  if (unlikely(!hwmon_cfg)) {                                                  \
    pr_loc_bug("Called %s without hwmon_cfg context being populated",          \
               __FUNCTION__);                                                  \
    return -EIO;                                                               \
  }
 ...
 *
 * Currently it always returns a fake value. However, it should only do so if
 * running under hypervisor. In bare-metal scenario we can simply proxy to
 * syno_cpu_temperature() [or not override that part at all].
 */
static int bios_get_cpu_temp(SYNOCPUTEMP *temp) {
  int fake_temp = prandom_int_range_stable(
      &cur_cpu_temp, TEMP_DEV, FAKE_CPU_TEMP_MIN, FAKE_CPU_TEMP_MAX);
  temp->cpu_num = MAX_CPU;
  for (int i = 0; i < MAX_CPU; ++i)
    temp->cpu_temp[i] = fake_temp;

  hwmon_pr_loc_dbg("mfgBIOS: GET_CPU_TEMP(surf=%d, cpuNum=%d) => %d°C",
                   temp->blSurface, temp->cpu_num, fake_temp);
...
static int bios_hwmon_get_thermal(SYNO_HWMON_SENSOR_TYPE *reading) {
  ...
  if (unlikely(!hwmon_thermals))
    kzalloc_or_exit_int(hwmon_thermals,
                        sizeof(int) * HWMON_SYS_THERMAL_ZONE_IDS);

  guarded_strscpy(reading->type_name, HWMON_SYS_THERMAL_NAME,
                  sizeof(reading->type_name));
  ...
  for (int i = 0; i < HWMON_SYS_THERMAL_ZONE_IDS; i++) {
    ...
    guarded_strscpy(
        reading->sensor[i].sensor_name,
        hwmon_sys_thermal_zone_id_map[hwmon_cfg->sys_thermal[i]],
        sizeof(reading->sensor[i].sensor_name)); // Save the name of the sensor
    hwmon_thermals[i] =
        prandom_int_range_stable(&hwmon_thermals[i], TEMP_DEV,
                                 FAKE_SURFACE_TEMP_MIN, FAKE_SURFACE_TEMP_MAX);
    snprintf(reading->sensor[i].value, sizeof(reading->sensor[i].value), "%d",
             hwmon_thermals[i]);
    ++reading->sensor_num;
...
static int bios_hwmon_get_voltages(SYNO_HWMON_SENSOR_TYPE *reading) {
  ...
  if (unlikely(!hwmon_voltages))
    kzalloc_or_exit_int(hwmon_voltages,
                        sizeof(int) * HWMON_SYS_VOLTAGE_SENSOR_IDS);
  ...
  for (int i = 0; i < HWMON_SYS_VOLTAGE_SENSOR_IDS; i++) {
    ...
    guarded_strscpy(
        reading->sensor[i].sensor_name,
        hwmon_sys_vsens_id_map[hwmon_cfg->sys_voltage[i]],
        sizeof(reading->sensor[i].sensor_name)); // Save the name of the sensor
    hwmon_voltages[i] = prandom_int_range_stable(
        &hwmon_voltages[i], VOLT_DEV, fake_volt_min(hwmon_cfg->sys_voltage[i]),
        fake_volt_max(hwmon_cfg->sys_voltage[i]));
    snprintf(reading->sensor[i].value, sizeof(reading->sensor[i].value), "%d",
             hwmon_voltages[i]);
    ++reading->sensor_num;

    hwmon_pr_loc_dbg("mfgBIOS: <= %s() %s->%d mV", __FUNCTION__,
                     hwmon_sys_vsens_id_map[hwmon_cfg->sys_voltage[i]],
                     hwmon_voltages[i]);
  }
...
static int bios_hwmon_get_fans_rpm(SYNO_HWMON_SENSOR_TYPE *reading) {
  ...
  guarded_strscpy(reading->type_name, HWMON_SYS_FAN_RPM_NAME,
                  sizeof(reading->type_name));
  hwmon_pr_loc_dbg("mfgBIOS: => %s(type=%s)", __FUNCTION__, reading->type_name);
  ...
  for (int i = 0; i < HWMON_SYS_FAN_RPM_IDS; i++) {
    ...
    hwmon_fans_rpm[i] = prandom_int_range_stable(
        &hwmon_fans_rpm[i], FAN_SPEED_DEV, FAKE_RPM_MIN, FAKE_RPM_MAX);
    snprintf(reading->sensor[i].value, sizeof(reading->sensor[i].value), "%d",
             hwmon_fans_rpm[i]);
    ++reading->sensor_num;
 ...
 *
 * Currently values here are just a guesstimation - we don't have a platform to
 * see the real values but based on their names it's assumed these are number of
 * detected and enabled disks. This probably should ask the SCSI driver for the
 * number of disks overall (as no PC architecture has any clue about number of
 * disks present physically if they don't register with the system). On a real
 * hardware it's probably checked by some contact switch/IR sensor to check if a
 * given slot for a disk isn't empty.
 *
 * @param reading Pointer to save results
 * @return 0 on success, -E on error
 */
static int bios_hwmon_get_hdd_backplane(SYNO_HWMON_SENSOR_TYPE *reading) {
  ...
  guarded_strscpy(reading->type_name, HWMON_HDD_BP_STATUS_NAME,
                  sizeof(reading->type_name));
  hwmon_pr_loc_dbg("mfgBIOS: => %s(type=%s)", __FUNCTION__, reading->type_name);
  ...
  for (int i = 0; i < HWMON_SYS_HDD_BP_IDS; i++) {
    ...
    guarded_strscpy(
        reading->sensor[i].sensor_name,
        hwmon_hdd_bp_id_map[hwmon_cfg->hdd_backplane[i]],
        sizeof(reading->sensor[i].sensor_name)); // Save the name of the sensor
    snprintf(reading->sensor[i].value, sizeof(reading->sensor[i].value), "%d",
             hdd_num);
    ++reading->sensor_num;
    ...
    hwmon_pr_loc_dbg("mfgBIOS: <= %s() %s->%d", __FUNCTION__,
                     hwmon_hdd_bp_id_map[hwmon_cfg->hdd_backplane[i]], hdd_num);
  }
 ...
 *
 * Currently this command is not implemented and always return an error as we
 * haven't yet seen any devices using it.
 *
 * @param reading Pointer to save results
 * @return 0 on success, -E on error
 */
static int bios_hwmon_get_psu_status(struct hw_config_hwmon *hwc,
                                     SYNO_HWMON_SENSOR_TYPE *reading) {
  pr_loc_wrn("mfgBIOS: **UNIMPLEMENTED** %s(type=%s)", __FUNCTION__,
             HWMON_PSU_STATUS_NAME);
 ...
 * @return 0 on success, -E on error
 */
static int bios_hwmon_get_current(struct hw_config_hwmon *hwc,
                                  SYNO_HWMON_SENSOR_TYPE *reading) {
  pr_loc_wrn("mfgBIOS: **UNIMPLEMENTED** %s(type=%s)", __FUNCTION__,
             HWMON_SYS_CURRENT_NAME);
  return -EIO; // todo: we haven't [yet] seen a device using this
}
...
int shim_bios_module_hwmon_entries(const struct hw_config *hw) {
  ...
  if (platform_has_hwmon_hdd_bpl(hw))
    _shim_bios_module_entry(VTK_GET_HWMON_HDD_BKPLANE,
                            bios_hwmon_get_hdd_backplane);

  if (platform_has_hwmon_psu_status(hw))
    _shim_bios_module_entry(VTK_GET_HWMON_PSU_STATUS,
                            bios_hwmon_get_psu_status);
