/**
 * Overrides GetHwCapability to provide additional capabilities for older
 * platforms (e.g. 3615xs)
 */
...
static void dbg_compare_cap_value(SYNO_HW_CAPABILITY id, int computed_support) {
  ...
  org_cap.id = id;
  int ovs_fout =
      call_overridden_symbol(org_fout, GetHwCapability_ovs, &org_cap);

  pr_loc_dbg("comparing GetHwCapability(id=%d)->support => computed=%d vs. "
             "real=%d [org_fout=%d, ovs_fout=%d]",
             id, computed_support, org_cap.support, org_fout, ovs_fout);
#endif
}
...
static int GetHwCapability_shim(CAPABILITY *cap) {
  ...
  case CAPABILITY_LCM: {
    if (unlikely(!GetHwCapability_ovs)) {
      pr_loc_bug("%s() was called with proxy need when no OVS was available",
                 __FUNCTION__);
      return -EIO;
    }
    ...
    pr_loc_dbg("proxying GetHwCapability(id=%d)->support => real=%d "
               "[org_fout=%d, ovs_fout=%d]",
               cap->id, cap->support, org_fout, ovs_fout);
...
int register_bios_hwcap_shim(const struct hw_config *hw) {
  ...
  hw_config = hw;
  override_symbol_or_exit_int(GetHwCapability_ovs, "GetHwCapability",
                              GetHwCapability_shim);
