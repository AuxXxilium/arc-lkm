...
#define SHIM_TO_NULL_ZERO_INT(for_what)                                        \
  _shim_bios_module_entry(for_what, bios_##for_what##_null_zero_int);

/********************************************* mfgBIOS LKM static shims
 * ***********************************************/
static unsigned long org_shimmed_entries[VTK_SIZE] = {
    '\0'}; // original entries which were shimmed by custom entries
static unsigned long cust_shimmed_entries[VTK_SIZE] = {
    '\0'}; // custom entries which were set as shims
 ...
 * *********************************************/
static unsigned long *vtable_start =
    NULL; // set when shim_bios_module is called()
void _shim_bios_module_entry(const unsigned int idx, const void *new_sym_ptr) {
  if (unlikely(!vtable_start)) {
    pr_loc_bug("%s called without vtable start populated - are you calling it "
               "outside of shim_bios_module scope?!",
               __FUNCTION__);
    return;
  }
  ...
  // The vtable entry is either not shimmed OR already shimmed with what we set
  // before OR already *was* shimmed but
  //  external (i.e. mfgBIOS) code overrode the shimmed entry.
  // We only save the original entry if it was set by the mfgBIOS (so not
  // shimmed yet or ext. override situation)

  // it was already shimmed and the shim is still there => noop
  if (cust_shimmed_entries[idx] &&
      cust_shimmed_entries[idx] == vtable_start[idx])
    return;

  pr_loc_dbg("mfgBIOS vtable [%d] originally %ps<%p> will now be %ps<%p>", idx,
             (void *)vtable_start[idx], (void *)vtable_start[idx], new_sym_ptr,
             new_sym_ptr);
...
/**
 * Prints a table of memory between vtable_start and vtable_end, trying to
 * resolve symbols as it goes
 */
static void print_debug_symbols(const unsigned long *vtable_end) {
  ...
  int im =
      vtable_end -
      vtable_start; // Should be multiplies of 8 in general (64 bit alignment)
...
bool shim_bios_module(const struct hw_config *hw, struct module *mod,
  ...
  if (unlikely(!vt_start || !vt_end)) {
    pr_loc_bug("%s called without vtable start or vt_end populated?!",
               __FUNCTION__);
    return false;
  }
  ...
  if (hw->emulate_rtc) {
    ...
    pr_loc_dbg("Native RTC supported - not enabling proxy (emulate_rtc=%d)",
               hw->emulate_rtc ? 1 : 0);
  }

  shim_bios_module_hwmon_entries(
      hw); // Shim all hardware environment stuff (temps, fans, etc.)
...
bool unshim_bios_module(unsigned long *vt_start, unsigned long *vt_end) {
  for (int i = 0; i < VTK_SIZE; i++) {
    // make sure to check the cust_ one as org_ may contain NULL ptrs and we
    // should restore them as NULL if they were
    //  so originally
...
/*
 * Syno kernel has ifdefs for "MY_ABC_HERE" for syno_ahci_disk_led_enable() and
 * syno_ahci_disk_led_enable_by_port() so we need to check if they really exist
 * and we cannot determine it statically
 */
/*
...
#if !defined(CONFIG_SYNO_EPYC7002) && !defined(CONFIG_SYNO_PURLEY)
/******************************** Kernel-level shims related to mfgBIOS
 * functionality *********************************/
extern void *funcSYNOSATADiskLedCtrl; // if this explodes one day we need to do
                                      // kernel_has_symbol() on it dynamically
...
static int funcSYNOSATADiskLedCtrl_shim(int host_num, SYNO_DISK_LED led) {
  ...
  // exit code is not used anywhere in the public code, so this value is an
  // educated guess based on libata-scsi.c
  return 0;
}
#endif

int syno_ahci_disk_led_enable_shim(const unsigned short host_num,
                                   const int value) {
  pr_loc_dbg("Received %s with host=%d val=%d", __FUNCTION__, host_num, value);
  return 0;
}

int syno_ahci_disk_led_enable_by_port_shim(const unsigned short port,
                                           const int value) {
...
int shim_disk_leds_ctrl(const struct hw_config *hw) {
  // we're checking this here to remove knowledge of "struct hw_config" from
  // bios_shim letting others know it's NOT the place to do BIOS shimming
  // decisions
  ...
  int out;
#if !defined(CONFIG_SYNO_EPYC7002) && !defined(CONFIG_SYNO_PURLEY)
  // funcSYNOSATADiskLedCtrl exists on (almost?) all platforms, but it's null on
  // some... go figure ;)
  if (funcSYNOSATADiskLedCtrl) {
    ov_funcSYNOSATADiskLedCtrl = override_symbol("funcSYNOSATADiskLedCtrl",
  ...
  if (kernel_has_symbol("syno_ahci_disk_led_enable")) {
    ov_syno_ahci_disk_led_enable = override_symbol(
        "syno_ahci_disk_led_enable", syno_ahci_disk_led_enable_shim);
  ...
  if (kernel_has_symbol("syno_ahci_disk_led_enable_by_port")) {
    ov_syno_ahci_disk_led_enable_by_port =
        override_symbol("syno_ahci_disk_led_enable_by_port",
                        syno_ahci_disk_led_enable_by_port_shim);
    if (unlikely(IS_ERR(ov_syno_ahci_disk_led_enable_by_port))) {
      ...
      pr_loc_err("Failed to shim syno_ahci_disk_led_enable_by_port, error=%d",
                 out);
      return out;
    }
  }
...
int unshim_disk_leds_ctrl(void) {
  ...
  if (ov_syno_ahci_disk_led_enable_by_port) {
    ...
    ov_syno_ahci_disk_led_enable_by_port = NULL;
    if (unlikely(out != 0)) {
      pr_loc_err("Failed to unshim syno_ahci_disk_led_enable_by_port, error=%d",
                 out);
      failed = true;
    }
  }
