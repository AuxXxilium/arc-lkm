...
#define shim_reg_already()                                                     \
  do {                                                                         \
    pr_loc_bug("Called %s while %s() shim is already", __FUNCTION__,           \
               SHIM_NAME);                                                     \
    return -EALREADY;                                                          \
  } while (0)
#define shim_ureg_in() pr_loc_dbg("Unregistering %s shim", SHIM_NAME);
#define shim_ureg_ok()                                                         \
  pr_loc_inf("Successfully unregistered %s shim", SHIM_NAME);
#define shim_ureg_nreg()                                                       \
  do {                                                                         \
    pr_loc_bug("Called %s() while %s shim is not registered (yet?)",           \
               __FUNCTION__, SHIM_NAME);                                       \
    return -ENXIO;                                                             \
  } while (0)
