...
static int noinline uart_swap_hw_output(unsigned int from, unsigned char to) {
  pr_loc_wrn("UART swapping needed for the platform but forcefully disabled "
             "via DBG_DISABLE_UART_SWAP");
  return 0;
}
...
static int noinline uart_swap_hw_output(unsigned int from, unsigned char to) {
  pr_loc_bug(
      "Called %s from uart_fixer context when UART_BUG_SWAPPED is not set",
      __FUNCTION__);
  return -EINVAL;
}
#endif

static bool ttyS0_force_initted =
    false;                          // Was ttyS0 forcefully initialized by us?
static bool serial_swapped = false; // Whether ttyS0 and ttyS1 were swapped

/**
 * On some platforms (e.g. 918+) the first serial port appears to not be
 * functional as it's not initialized properly.
 *
 * It is speculated that it has to do with
 * "CONFIG_SYNO_X86_TTY_CONSOLE_OUTPUT=y" but it's not confirmed. If this is not
 * fixed by this function setting kernel console output to ttyS0 will result in
 * earlycon working as expected (as it doesn't use the normal 8250 driver) with
 * nothing being transmitted as soon as earlycon is switched to the proper
 * "console=" port.
 */
static int fix_muted_ttyS0(void) {
  int out = 0;
  struct uart_port port = {.iobase = STD_COM1_IOBASE,
...
int register_uart_fixer(const hw_config_uart_fixer *hw) {
  ...
  int out = 0;
  if ((hw->swap_serial && (out = uart_swap_hw_output(1, 0)) != 0) ||
...
int unregister_uart_fixer(void) {
  ...
  int out = 0;
  if ((serial_swapped && (out = uart_swap_hw_output(0, 1)) != 0) ||
