  ...
  shost_for_each_device(sdev, shost) {
    if (strcmp(blk_name, sdev->syno_disk_name) == 0) {
      pr_loc_dbg("scsi host no: %d, device id: %d, name: %s, serial: %s",
                 shost->host_no, sdev->id, sdev->syno_disk_name,
...
struct Scsi_Host *rp_search_scsi_host_by_blk_name(struct class *shost_class,
                                                  char *blk_name) {
  struct device *cdev;
  struct Scsi_Host *shost = NULL;

  cdev = class_find_device(shost_class, NULL, &blk_name,
                           rp_scsi_device_disk_name_match);
  if (cdev) {
