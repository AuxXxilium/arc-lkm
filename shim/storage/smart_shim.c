...
static void set_ata_string(u8 *dst, const char *src, u8 length)
{
    ...
    for (u8 i = 0; i < length; i += 2)
    {
        ...
        /* src has an odd number of characters before its NUL terminator (e.g. "2.0"): the second
         * byte of this word-pair is the terminator itself, not a real character. Place the lone
         * trailing character at dst[i+1] (matching the src[i]->dst[i+1] mapping used for full pairs
         * below) and leave dst[i] as the space padding memset() above already wrote, then stop -
         * continuing the loop would read src[i+2] onward, which is past the end of the string's
         * allocation (undefined data, not padding), and previously got byte-swapped into the field
         * as garbage. */
        if (src[i + 1] == '\0') {
            dst[i + 1] = src[i];
            break;
 ...
 *
 * @param field ATA string field to sanitize in place
                                   ...
                                   const char *const disk_name, const char *const disk_fw_rev)
{
    ...
    struct rp_hd_driveid *real_did = (void *)ata_identity;

    /* Some virtualized SATA/SCSI controllers (e.g. VMware's) hand back a genuine ATA IDENTIFY
     * response for their virtual disks, which - unlike the fully-synthetic populate_ata_id() path
     * above - is never generated by this module. Malformed padding in the *real* device's own
     * firmware-revision field (non-space garbage past the actual version string) previously reached
     * DSM/hdddb.sh unmodified, showing up as e.g. "2.0 ????" in Storage Manager. Sanitize it
     * regardless of the SMART branch taken below, since both copy this buffer back to userspace. */
    sanitize_ata_string(real_did->fw_rev, sizeof(real_did->fw_rev));

    if (ata_is_smart_supported(ata_identity) && ata_is_smart_enabled(ata_identity)) {
        pr_loc_dbg("ATA_CMD_ID_ATA confirmed SMART support - only fw_rev sanitized");
    } else {
        //if SMART is not supported we modify the response, retaining the original response header but
        // changing SMART flags
    ...
    //recalculate checksum once, after any of the above modifications
