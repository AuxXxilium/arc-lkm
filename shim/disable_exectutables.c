...
int register_disable_executables_shim(void) {
  ...
  int out;
  if ((out = add_blocked_execve_filename(BOOTLOADER_UPDATE1_PATH)) != 0 ||
...
int unregister_disable_executables_shim(void) {
  // noop - execve entries will be cleared in one sweep during unregister of
  // interceptor (it's much faster this way) this function is kept for
  // consistency
  return 0;
}
