...
#define PMU_TTYS_LINE                                                          \
  1 // so far this is hardcoded by syno, so we doubt it will ever change
#define WORK_BUFFER_LEN VUART_FIFO_LEN
#define to_hex_buf_len(len)                                                    \
  ((len) * 3 + 1) // 2 chars for each hex + space + NULL terminator
#define HEX_BUFFER_LEN to_hex_buf_len(VUART_FIFO_LEN)

// PMU packets are at minimum 2 bytes long (PMU_CMD_HEAD + 1-3 bytes command +
// optional data). If this is set to a high
//  value (e.g. VUART_FIFO_LEN) in practice commands will only be delivered when
//  the client indicates end-of-transmission) which may not be bad...
#define PMU_MIN_PACKET 2
#define PMU_CMD_HEAD                                                           \
  0x2d // every PMU packet is delimited by containing 0x2d (ASCII "-"/dash) as
       // its first character
 ...
 */
static void cmd_shim_noop(const command_definition *t, const char *data,
                          u8 data_len) {
  pr_loc_dbg("vPMU received %s using %d bytes - NOOP", t->name, data_len);
}

//@todo when we get the physical PMU emulator we can move this to a separate
// library so that shim contacts an internal
// routing routine for commands which aren't shimmed here. Then we will add all
// PMU=>kernel commands as well. Currently we only define kernel=>PMU ones as
// these are the ones we need to listen for.
#define PMU_CMD__MIN_CODE 0x30
#define PMU_CMD__MAX_CODE 0x75
#define single_byte_idx(id) ((id) - PMU_CMD__MIN_CODE)
#define get_single_byte_cmd(id)                                                \
  single_byte_cmds[single_byte_idx(                                            \
      id)] // call it ONLY after has_single_byte_cmd!!!
#define has_single_byte_cmd(id)                                                \
  (likely((id) >= PMU_CMD__MIN_CODE) && likely((id) <= PMU_CMD__MAX_CODE) &&   \
   get_single_byte_cmd(id).length != 0)
#define DEFINE_SINGLE_BYTE_CMD(cnm, fp)                                        \
  [single_byte_idx(PMU_CMD_##cnm)] = {.name = #cnm, .length = 1, .fn = fp}
...
// 0x4C unknown
#define PMU_CMD_OUT_LED_TOG_PWR_STAT                                           \
  0x4d //"M", allows for using one led for status and power and toggle between
       // them
// 0x4E unknown
...
static const command_definition
    single_byte_cmds[single_byte_idx(PMU_CMD__MAX_CODE) + 1] = {
...
static char *uart_buffer = NULL; // keeps data streamed directly by the vUART...
                                 // todo: vUART should manage this buffer
static char *work_buffer =
    NULL; // collecting & operatint on the data received from vUART
static char *work_buffer_curr =
    NULL; // pointer to the current free space in work_buffer
static char *hex_print_buffer =
    NULL; // helper buffer to print char arrays in hex
 ...
 *
 * It is safe to call this method without checking buffers state (it has a
 * deliberate protection against double-free)
 */
...
static __used const char *get_hex_print(const char *buffer, int len) {
  if (unlikely(len == 0)) {
    ...
    pr_loc_bug("Printing %d bytes as hex requires %d bytes in buffer - buffer "
               "is %d bytes",
               len, to_hex_buf_len(len), HEX_BUFFER_LEN);
    hex_print_buffer[0] = '\0';
    return hex_print_buffer;
  }
 ...
 *
 * @param cmd pointer to a pointer where address of command structure can be
 * saved if found
 */
static pmu_match_status noinline match_command(const command_definition **cmd,
                                               const char *signature,
                                               const unsigned int sig_len) {
  if (unlikely(sig_len == 0)) {
    pr_loc_dbg("Invalid zero-length command (stray head without command "
               "signature) - discarding");
    return PMU_CMD_NOT_FOUND;
  }

  if (likely(sig_len == 1) // regular 1 byte
      || (sig_len == 3 && signature[1] == 0x0d &&
          signature[2] == 0x0a) // 1 byte with CRLF (sic!)
  ) {
  ...
  return PMU_CMD_NOT_FOUND; //@todo Currently we don't handle multibyte
                            // commands; it has to be a full iteration
}
...
static void route_command(const char *buffer, const unsigned int len) {
  ...
  if (match_command(&cmd, buffer, len) != PMU_CMD_FOUND) {
    pr_loc_wrn(
        "Unknown %d byte PMU command with signature hex=\"%s\" ascii=\"%.*s\"",
        len, get_hex_print(buffer, len), len, buffer);
    return;
  }
 ...
 *
 * @param end_of_packet Indicates whether this command was called because the
 * vUART transmitter assumed end-of-transmission/IDLE, or flushed due to its
 * buffer being full. If this parameter is true the data in the work buffer is
 * assumed to be a complete representation of the state. This becomes important
 * if we cannot with 100% confidence say, after reaching the end of the buffer,
 * if it will be a multibyte command (but we possibly didn't get all the bytes
 * YET) or this is single or multibyte command which we don't know.
 *
 * This function makes an assumption that when it's called that the buffer
 * represents the whole state of the command If this becomes to take too long we
 * can move it to a separate thread, but this will require a lock.
 */
static noinline void process_work_buffer(bool end_of_packet) {
  if (unlikely(work_buffer == work_buffer_curr)) {
    // this can happen if kernel sends no data but we get IDLE... shouldn't
    // logically happen
  ...
  for (char *curr = work_buffer; curr < work_buffer_curr; ++curr) {
    if (*curr == PMU_CMD_HEAD) { // got the beginning of a new command
      // we've found a new command in the buffer - lets check if the previously
      // collected data matches anything
      if (cmd_len != -1) { // we only want to call it if this isn't the first
                           // byte after last cmd (or 1st in buf)
        route_command(curr - cmd_len, cmd_len);
        cmd_len = 0; // we've got the head so 0 and not -1
      } else {
        ++cmd_len; // We've read the buffer containing head, we then expect to
                   // get something which is non-head
      }
    } else {
      if (cmd_len == -1) { // we don't expect data before head
        pr_loc_wrn("Found garbage data in PMU buffer before cmd head (\"%c\" / "
                   "0x%02x) - ignoring",
                   *curr, *curr);
        continue;
      }
  ...
  // We've finished processing the buffer. Now we need to decide what to do with
  // that last piece of data
  unsigned int processed = work_buffer_fill();
  if (cmd_len != -1) { // if it's -1 it means we didn't find any heading so
                       // we're just discarding all data
    if (end_of_packet) {
      route_command(work_buffer_curr - cmd_len, cmd_len);
    } else { // if the packed didn't end we need to keep that piece of buffer
             // for the next run
  ...
  //    pr_loc_dbg("Left buffer %p curr=%p with %d bytes in it (ascii=\"%.*s\"
  //    hex={%s})", work_buffer, work_buffer_curr, left, left, work_buffer,
 ...
 */
static noinline void pmu_rx_callback(int line, const char *buffer,
                                     unsigned int len,
                                     vuart_flush_reason reason) {
  pr_loc_dbg("Got %d bytes from PMU: reason=%d hex={%s} ascii=\"%.*s\"", len,
             reason, get_hex_print(buffer, len), len, buffer);

  int buffer_space = WORK_BUFFER_LEN - work_buffer_fill();
  if (unlikely(work_buffer_curr + len >
               work_buffer +
                   WORK_BUFFER_LEN)) { // todo just remove as much as needed
                                       // from the buffer to fit more data
    pr_loc_err(
        "Work buffer is full! Only %d of %d bytes will be copied from receiver",
        len, buffer_space);
    len = buffer_space;
  }
  ...
  //    pr_loc_dbg("Copied data to work buffer, now with %d bytes in it
  //    (cur=%p)",
  //               (unsigned int)(work_buffer_curr - work_buffer),
  //               work_buffer_curr);

  // We only want to analyze the buffer when we are sure we have the full
  // command to process. This is because commands
  //  are variable length and have no end delimiter not length specified with
  //  prefixes of short commands conflicting with longer commands (sic!)
  // For example, you have "SW1" command which when sent will look like "-SW1"
  // (0x2d 0x53 0x57 0x31). We can capture
  //  this when VUART_FLUSH_IDLE happens. We can also easily capture this when
  //  multiple commands are sent at once (unlikely but possible) since it will
  //  be something like "-SW1-3". However, we CANNOT distinguish "-S" from
  //  incomplete "-SW1". So we need to rely on IDLE - if we got "-S" with IDLE
  //  it means it was "-S" and not the beginning of "-SW1".
  // We also forcefully flush on full buffer even if no IDLE was specified, as
  // it's technically possible for the
  //  software to send a long sequence of commands at once totaling more than
  //  our buffer (extremely unlikely).
  // Additionally, we only process IDLE-signalled buffers when they have at
  // least a single byte of data as some
  //  versions of the mfgBIOS attach head AND THEN in a separate packet send the
  //  actual commands (sic!)
  if (reason == VUART_FLUSH_IDLE && work_buffer_fill() > 1)
    process_work_buffer(true);
  // our buffer is full [we must process] or vUART buffer was full [we should
  // process]
...
int register_pmu_shim(const struct hw_config *hw) {
  ...
  // We don't set the threshold as some commands are variable length but the
  // "packets" are properly split
  if ((out = vuart_set_tx_callback(PMU_TTYS_LINE, pmu_rx_callback, uart_buffer,
                                   VUART_THRESHOLD_MAX))) {
