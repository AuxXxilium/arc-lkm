...
void *get_shimmed_boot_dev(void) { return mapped_shim_data; }
...
bool scsi_is_boot_dev_target(const struct boot_media *boot_dev_config,
  ...
  pr_loc_dbg("Checking if SATA disk is a shim target - id=%u channel=%u "
             "vendor=\"%s\" model=\"%s\"",
             sdp->id, sdp->channel, sdp->vendor, sdp->model);

  long long capacity_mib = opportunistic_read_capacity(sdp);
  if (unlikely(capacity_mib < 0)) {
    pr_loc_dbg("Failed to estimate drive capacity (error=%lld) - it WILL NOT "
               "be shimmed",
               capacity_mib);
    return false;
  }
  ...
  if (capacity_mib > boot_dev_config->dom_size_mib) {
    pr_loc_dbg(
        "Device has capacity of ~%llu MiB - it WILL NOT be shimmed (>%lu)",
        capacity_mib, boot_dev_config->dom_size_mib);
    return false;
  }

  if (unlikely(get_shimmed_boot_dev())) {
    pr_loc_wrn("Boot device was already shimmed but a new matching device "
               "(~%llu MiB <= %lu) appeared again - "
  ...
  pr_loc_dbg(
      "Device has capacity of ~%llu MiB - it is a shimmable target (<=%lu)",
      capacity_mib, boot_dev_config->dom_size_mib);
