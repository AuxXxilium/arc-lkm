 ...
 *
 * Internally in the kernel code it is available as "saved_command_line".
 * However that variable is not accessible for modules. This function populates
 * a char buffer with the cmdline extracted using other methods.
 *
 * WARNING: if something (e.g. sanitize cmdline) overrides the cmdline this
 * method will return the overridden one! However, this method caches the
 * cmdline, so if you call it once it will cache the original one internally.
 *
 ...
 *
 * Note: it's not guaranteed that the config will be valid. Check
 * runtime_config.h.
 *
