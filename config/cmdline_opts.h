...
#define CMDLINE_MAX                                                            \
  1024 // Max length of cmdline expected/processed; if longer a warning will be
       // emitted
#define CMDLINE_SEP "\t\n "
 ...
 *
 * All should be defined in the .h to allow accessing outside for hints in
 * errors.
 */
#define CMDLINE_CT_VID "vid=" // Boot media Vendor ID override
#define CMDLINE_CT_PID "pid=" // Boot media Product ID override
#define CMDLINE_CT_MFG                                                         \
  "mfg" // VID & PID override will use force-reinstall VID/PID combo
#define CMDLINE_CT_DOM_SZMAX                                                   \
  "dom_szmax=" // Max size of SATA device (MiB) to be considered a DOM (usually
               // you should NOT use this)

// Standard Linux cmdline tokens
#define CMDLINE_KT_ELEVATOR                                                    \
  "elevator=" // Sets I/O scheduler (we use it to load RP LKM earlier than
              // normally possible)
#define CMDLINE_KT_LOGLEVEL "loglevel="
#define CMDLINE_KT_PK_BUFFER                                                   \
  "log_buf_len=" // Length of the printk ring buffer (should usually be
                 // increased for debug)
...
// 0|1 - whether to use native SATA Disk-on-Module for boot drive (syno); 2 -
// use fake/emulated SATA DOM (rp)
...
// You CANNOT simply add more macN= - DSM kernel only uses 4. If they ever
// support >4 you need to modify cmdline handling
