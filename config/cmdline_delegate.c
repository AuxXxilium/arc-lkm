...
#define ensure_cmdline_param(cmdline_param)                                    \
  if (strncmp(param_pointer, cmdline_param, strlen_static(cmdline_param)) !=   \
      0) {                                                                     \
    return false;                                                              \
  }

#define ensure_cmdline_token(cmdline_param)                                    \
  if (strncmp(param_pointer, cmdline_param, sizeof(cmdline_param)) != 0) {     \
    return false;                                                              \
  }
...
static bool extract_hw(syno_hw *model, const char *param_pointer) {
  ...
  if (strscpy((char *)model, param_pointer + strlen_static(CMDLINE_KT_HW),
              sizeof(syno_hw)) < 0)
...
static bool extract_sn(serial_no *sn, const char *param_pointer) {
  ...
  if (strscpy((char *)sn, param_pointer + strlen_static(CMDLINE_KT_SN),
              sizeof(serial_no)) < 0)
...
static bool extract_boot_media_type(struct boot_media *boot_media,
                                    const char *param_pointer) {
  ...
  case CMDLINE_KT_SATADOM_DISABLED:
    ...
    pr_loc_wrn(
        "SATA-based boot media disabled (default will be used, %s0 is a noop)",
        CMDLINE_KT_SATADOM);
    break;

  default:
    pr_loc_err("Option \"%s%c\" is invalid (value should be 0/1/2)",
               CMDLINE_KT_SATADOM, value);
  }
...
static bool extract_vid(device_id *user_vid, const char *param_pointer) {
  ...
  int tmp_call_res = kstrtoll(param_pointer + strlen_static(CMDLINE_CT_VID), 0,
                              &numeric_param);
  ...
  if (unlikely(numeric_param > VID_PID_MAX)) {
    pr_loc_err("Cmdline %s is invalid (value larger than %d)", CMDLINE_CT_VID,
               VID_PID_MAX);
    return true;
  }

  if (unlikely(*user_vid) != 0)
    pr_loc_wrn("VID was already set to 0x%04x by a previous instance of %s - "
               "it will be changed now to 0x%04x",
...
static bool extract_pid(device_id *user_pid, const char *param_pointer) {
  ...
  long long numeric_param;
  int tmp_call_res = kstrtoll(param_pointer + strlen_static(CMDLINE_CT_PID), 0,
                              &numeric_param);
  if (unlikely(tmp_call_res != 0)) {
    pr_loc_err("Call to %s() failed => %d", "kstrtoll", tmp_call_res);
    return true;
  ...
  if (unlikely(numeric_param > VID_PID_MAX)) {
    pr_loc_err("Cmdline %s is invalid (value larger than %d)", CMDLINE_CT_PID,
               VID_PID_MAX);
    return true;
  }

  if (unlikely(*user_pid) != 0)
    pr_loc_wrn("PID was already set to 0x%04x by a previous instance of %s - "
               "it will be changed now to 0x%04x",
...
/**
 * Extracts maximum size of SATA DOM (dom_szmax=<number of MiB>) from kernel cmd
 * line
 */
static bool extract_dom_max_size(struct boot_media *boot_media,
                                 const char *param_pointer) {
  ensure_cmdline_param(CMDLINE_CT_DOM_SZMAX);

  long size_mib = simple_strtol(
      param_pointer + strlen_static(CMDLINE_CT_DOM_SZMAX), NULL, 10);
  if (size_mib <= 0) {
    pr_loc_err("Invalid maximum size of SATA DoM (\"%s=%ld\")",
               CMDLINE_CT_DOM_SZMAX, size_mib);
    return true;
  }
...
static bool extract_port_thaw(bool *port_thaw, const char *param_pointer) {
  ...
  if (value == '\0') {
    pr_loc_err("Option \"%s%d\" is invalid (value should be 0 or 1)",
               CMDLINE_KT_THAW, value);
    return true;
  }
...
/**
 * Extracts number of expected network interfaces (netif_num=<number>) from
 * kernel cmd line
 *
 ...
 * @return true on match, false if param didn't match
 */
static bool extract_netif_num(unsigned short *netif_num,
                              const char *param_pointer) {
  ensure_cmdline_param(CMDLINE_KT_NETIF_NUM);

  short value = *(param_pointer + strlen_static(CMDLINE_KT_NETIF_NUM)) -
                48; // ASCII: 0=48 and 9=57

  if (value == 0) {
    pr_loc_wrn("You specified no network interfaces (\"%s=0\")",
               CMDLINE_KT_NETIF_NUM);
    return true;
  }

  if (value < 1 || value > 9) {
    pr_loc_err("Invalid number of network interfaces set (\"%s%d\")",
               CMDLINE_KT_NETIF_NUM, value);
    return true;
  }
...
/**
 * Extracts network interfaces MAC addresses (mac1...mac4=<MAC> **OR**
 * macs=<mac1,mac2,macN>)
 *
 ...
 * @return true on match, false if param didn't match
 */
static bool extract_netif_macs(mac_address *macs[MAX_NET_IFACES],
                               const char *param_pointer) {
  if (strncmp(param_pointer, CMDLINE_KT_MACS, strlen_static(CMDLINE_KT_MACS)) ==
      0) {
    unsigned short i = 0;
    ...
    while (NULL != pEnd && MAX_NET_IFACES > i) {
      *pEnd = '\0';
      ...
      if (unlikely(!macs[i])) {
        pr_loc_crt("kernel memory alloc failure - tried to allocate %lu bytes "
                   "for macs[%d]",
                   sizeof(mac_address), i);
        goto out_found;
      }
  ...
  // mac1=...mac8= are valid options. ASCII for 1 is 49, ASCII for
  // MAX_NET_IFACES is (49 + (MAX_NET_IFACES - 1)) # MAX_NET_IFACES must <=9
  if (strncmp(param_pointer, "mac", 3) != 0 || *(param_pointer + 4) != '=' ||
      *(param_pointer + 3) < 49 ||
  ...
  for (; i < MAX_NET_IFACES; i++) {
    ...
    if (strscpy((char *)macs[i], param_pointer + strlen_static(CMDLINE_KT_MAC1),
  ...
  pr_loc_err("You set more than MAC addresses! Only first %d will be honored.",
             MAX_NET_IFACES);
 ...
 *
 * The method we use here may seem weird but it is, believe or not, the most
 * direct one available. Kernel cmdline internally is stored in a
 * "saved_command_line" variable (and few derivatives) which isn't exported for
 * modules in any way (at least on x86). The only semi-direct way to get it is
 * to call the method responsible for /proc/cmdline)
 */
static int extract_kernel_cmdline(void) {
  struct seq_file cmdline_itr = {.buf = cmdline_cache, .size = CMDLINE_MAX};
 ...
 * @param cmdline_out
 * @param maxlen
 * @return 0 on success, -E2BIG if buffer was too small (but the operation
 * succeeded up to maxlen), -E for error
 */
...
#define ADD_BLACKLIST_ENTRY(idx, token)                                        \
  kmalloc_or_exit_int(cmdline_blacklist[idx], sizeof(token));                  \
  strcpy((char *)cmdline_blacklist[idx], token);                               \
  pr_loc_dbg("Add cmdline blacklist \"%s\" @ %d",                              \
             (char *)cmdline_blacklist[idx], idx);
int populate_cmdline_blacklist(
    cmdline_token *cmdline_blacklist[MAX_BLACKLISTED_CMDLINE_TOKENS],
    syno_hw *model) {
  // Currently, this list is static. However, it's prepared to be dynamic based
  // on the model Make sure you don't go over MAX_BLACKLISTED_CMDLINE_TOKENS
  // (and if so adjust it)
...
#ifndef NATIVE_SATA_DOM_SUPPORTED // on kernels without SATA DOM support we
                                  // shouldn't reveal that it's a SATA DOM-boot
...
int extract_config_from_cmdline(struct runtime_config *config) {
  ...
  while ((single_param_chunk = strsep(&cmdline_txt, CMDLINE_SEP)) != NULL) {
    if (unlikely(single_param_chunk[0] ==
                 '\0')) // Skip empty params (e.g. last one)
      continue;
